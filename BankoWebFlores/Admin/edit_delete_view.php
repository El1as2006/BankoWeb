				...
				<div class="container-fluid">
					...
					<div class="col-12">
						<div class="card">
							...
							<form action="edit_delete_view.php" method="POST">
								<div class="card-body">
									<div class="row">
							...
							<div class="table-responsive">
								<table class="table table-striped">
									...
									<tbody>
    <?php
    // Inicializa la variable $i como un array vacío
    $i = [];

    // Comprueba si se ha enviado una solicitud POST
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Verifica si el campo 'letra' está vacío
        ...
        if (!empty($i)) {
            foreach ($i as $data) { ?>
                <tr>
                    ...
                    <td class="d-none d-xl-table-cell">
                        <?php echo htmlspecialchars(decryptPayload($data['dui'])); ?>
                    </td>
                    <td class="d-none d-md-table-cell">
                        <?= htmlspecialchars(decryptPayload($data['email'])); ?>
                    </td>
                    <td class="table-action">
                        <a href="edituser_view.php?id=<?php echo urlencode($data['id']); ?>" class="btn btn-outline-primary"><?= Lang("Edit") ?></a>
                    </td>
                    <td class="table-action">
                        <a href="../deleteuser.php?id=<?php echo urlencode($data['id']); ?>" class="btn btn-outline-primary"><?= Lang("Delete") ?></a>
                    </td>
                    <td class="table-action">
                        <a href="../deactivateuser.php?id=<?php echo urlencode($data['id']); ?>" class="btn btn-outline-primary"><?= Lang("Desactivate") ?></a>
                    </td>
                    <td class="table-action">
                        <a href="../activateuser.php?id=<?php echo urlencode($data['id']); ?>" class="btn btn-outline-primary"><?= Lang("Activate") ?></a>
                    </td>
                </tr>
            <?php } 
        } else {
